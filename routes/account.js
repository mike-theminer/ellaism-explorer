...
router.get('/:account', function(req, res, next) {
  ...
  async.waterfall([
    function(callback) {
      ...
      if (data.lastBlock > 0x800) {
        data.fromBlock = data.lastBlock - 0x800;
      } else {
        data.fromBlock = 0x00;
      }
    ...
    data.tracesReceived.forEach(function(trace) {
      ...
      console.log(trace.blockNumber);
      if(trace.type === "reward" && blocks[trace.blockNumber][blocks[trace.blockNumber].length-1].type==="reward" ){
        console.log("duplicate reward");
        return;
      }
      blocks[trace.blockNumber].push(trace);
    });
